...
% -- Add Defaults
%% Implementation
function [refocusedImage, shiftedLightField] = lfRefocusMOD(lightField,arrayLength,arrayDepth,shiftMat,depth)
...
for i=1:arrayLength
    for j=1:arrayDepth
        index = index + 1;
        translatedImage = imtranslate(lightField(:,:,:,index),[-1*d*shiftMat(i,j,1),-1*d*shiftMat(i,j,2)]);
        shiftedLightField(:,:,:,index) = translatedImage;
        refocusedImage = refocusedImage + double(imgaussfilt(translatedImage,1));
    end
end
